...
int main(int argc, char* argv[])
{
    ...
    std::cout
        << "K=" << K << std::endl
        << "D=" << D << std::endl
        << "graySize width*height" << graySize.width << "*" << graySize.height << std::endl;

    cv::FileStorage file_kd("fisheye_KD", cv::FileStorage::WRITE);
    file_kd << "K_mat" << K;
    file_kd << "D_mat" << D;
    file_kd.release();
    ...
    cv::FileStorage file("fisheye_map", cv::FileStorage::WRITE);
    file << "map1" << map1;
    file << "map2" << map2;
    file.release();
